...
def configure_litellm_provider(provider: str) -> str:
    ...
    ----------
    provider:
        Name of the provider. Supported values: ``"openai"`` and
        ``"chatanywhere"``. Defaults to ``"openai"`` if empty.

    Returns
    -------
    str
        The normalised provider name that has been applied.
    """
    ...
    if provider_key == "chatanywhere":
        ...
        # Mirror the values so OpenAI-compatible SDKs continue to work.
...
def load_sandbox_fusion_config() -> SandboxFusionConfig:
    """Load SandboxFusion credentials from the environment."""

    _maybe_load_dotenv()
    api_key = os.getenv("SANDBOX_FUSION_API_KEY")
    if not api_key:
        raise RuntimeError(
            "SANDBOX_FUSION_API_KEY is not set. Please supply credentials for SandboxFusion."
        )
